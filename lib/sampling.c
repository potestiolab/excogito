...
void accept_move_spins(int rem_add[2], cg_mapping* mapping, cg_mapping* new_mapping, traj* Trajectory) {                              //ADDED COMPLETELY
    /**
    * routine that accepts a Simulated Annealing move. It updates all the relevant observables.
    *
    * Parameters
...
double tzero_estimation_spins(traj* Trajectory, int cgnum, int rsd, int verbose, int kl_flag, FILE* f_out_l) {		                            //(!) ADDED COMPLETELY
    /**
    * routine that makes *nrun* unbiased moves. It is used to estimate the starting temperature for Simulated Annealing.
    *
    * Parameters
...
void simulated_annealing_spins(traj* Trajectory, MC_params* SA_params, int cgnum, int rsd, int verbose, int kl_flag, FILE* f_out_l, int core) {     //ADDED COMPLETELY
    /**
    * simulated annealing optimisation
    * Parameters
    * ----------
    ...
    */
    ...
    for (mc = 0; mc < SA_params->MC_steps; mc++) {
        ...
        if ((new_mapping->smap - mapping->smap) < 0) {
            fprintf(f_out_l, "move accepted\n");
            accept_move_spins(rem_add, mapping, new_mapping, Trajectory);
            // check lowest
            if (mapping->smap < lowest_mapping->smap) {
                fprintf(f_out_l, "Lowest mapping entropy found\n");
                update_mapping(mapping, lowest_mapping, Trajectory->frames);
    ...
    sprintf(out_filename, "./MappingMin_N%d_ncg%d_MC%d_c%d.dat", mapping->n_at, cgnum, SA_params->MC_steps, core);
    FILE* f_smap;
