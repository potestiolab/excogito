...
void optimize_spins(arguments* arguments, parameters* cc) {
    ...
    */
    ...
    Trajectory->pairs = cc->frames * (cc->frames - 1) / 2;                                                              //(!) MODIFIED             
    printf("frames = %d\n", Trajectory->frames);
...
#pragma omp for schedule(static, 1)
            for (q = 0; q < nthreads; q++) {
                //sprintf(out_filename, "./OUTPUT_FILES/%skl_fast_delta_N%d_%d.dat", arguments->prot_code, cc->cgnum, q);    //(!) MODIFICARE??
                sprintf(out_filename, "./%skl_fast_delta_N%d_%d.dat", arguments->prot_code, cc->cgnum, q);    //(!) MODIFICARE??
                FILE* f_out_l;
                f_out_l = open_file_w(out_filename);
                tzeros[q] = tzero_estimation_spins(Trajectory, cc->cgnum, cc->rsd, arguments->verbose, 1, f_out_l);
...
#pragma omp for schedule(static, 1)
        for (q = 0; q < nthreads; q++) {
            ...
            printf("rsd = %d\n", cc->rsd);
            simulated_annealing_spins(Trajectory, SA_params, cc->cgnum, cc->rsd, arguments->verbose, 1, f_out_l, q);
            fclose(f_out_l);
            // free SA_params
            free(SA_params);
        }
